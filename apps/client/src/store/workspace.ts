...
const defaultSection: Section = {
  ...
  name: 'General',
  permanent: true,
  tabs: [{ id: 'tab-1', name: 'Canvas 1', sectionId: 'default', color: nextSwatchId([]) }],
  activeTabId: 'tab-1',
  color: nextSwatchId([]),
}
  ...
  persist(
    ...
    }),
    {
      ...
      migrate: (persistedState: any, version) => {
        ...
        if (version < 2) {
          // Backfill colors so existing workspaces don't render as a wall of grey.
          const usedSection: (string | null | undefined)[] = []
          const usedTab: (string | null | undefined)[] = []
          persistedState.sections = (persistedState.sections ?? []).map((sec: any) => {
            if (sec.color === undefined || sec.color === null) {
              sec.color = nextSwatchId(usedSection)
            }
            usedSection.push(sec.color)
            sec.tabs = (sec.tabs ?? []).map((t: any) => {
              if (t.color === undefined || t.color === null) {
                t.color = nextSwatchId(usedTab)
              }
              usedTab.push(t.color)
              return t
            })
            return sec
          })
          if (persistedState.colorsEnabled === undefined) {
            persistedState.colorsEnabled = true
          }
        }
