...
async fn get<R: Runtime>(_app: tauri::AppHandle<R>, args: GetArgs) -> Result<GetReturn, Error> {
    let value = {
        #[cfg(mobile)]    { mobile::get(&_app, &args.key).await.map_err(|e| Error::Other(e.to_string()))? }
...
async fn set<R: Runtime>(_app: tauri::AppHandle<R>, args: SetArgs) -> Result<(), Error> {
    #[cfg(mobile)]    { mobile::set(&_app, &args.key, &args.value).await.map_err(|e| Error::Other(e.to_string())) }
...
async fn delete<R: Runtime>(_app: tauri::AppHandle<R>, args: GetArgs) -> Result<(), Error> {
    #[cfg(mobile)]    { mobile::delete(&_app, &args.key).await.map_err(|e| Error::Other(e.to_string())) }
...
async fn has<R: Runtime>(_app: tauri::AppHandle<R>, args: GetArgs) -> Result<HasReturn, Error> {
    let v = {
        #[cfg(mobile)]    { mobile::has(&_app, &args.key).await.map_err(|e| Error::Other(e.to_string()))? }
    ...
    Builder::new("1scratch-secure-store")
        ...
        .setup(|app, _api| {
            #[cfg(target_os = "android")] {
                let h = _api.register_android_plugin("app.scratch.securestore", "SecureStorePlugin")?;
                app.manage(mobile::SecureStoreHandle(h));
            }
            #[cfg(target_os = "ios")] {
                extern "C" {
                    fn init_plugin_secure_store(webview: tauri::ipc::Channel<()>) -> *const std::ffi::c_void;
                }
                let h = _api.register_ios_plugin(init_plugin_secure_store)?;
                app.manage(mobile::SecureStoreHandle(h));
            }
            let _ = app;
            Ok(())
